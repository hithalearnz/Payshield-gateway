...
public class OpenApiConfig {
	...
	public OpenAPI distributedRateLimiterOpenApi() {
		return new OpenAPI()
				...
				.info(new Info().title("PayShield Gateway API").description(
	...
	/**
	 * SpringDoc may add an inferred host (e.g. http://localhost); keep only
	 * same-origin "/" for Try it out.
	 */
	@Bean
	public OpenApiCustomizer relativeServerOnly() {
		return openApi -> openApi
				.setServers(List.of(new Server().url("/").description("Current origin (preserve port)")));
	}
}
